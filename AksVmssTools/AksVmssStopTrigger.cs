...
{
    public static class AksVmssStopTrigger
    {
        [FunctionName("AksVmssStopTrigger")]
        public static async Task Run([TimerTrigger("%VmssStopTriggerSchedule%")]TimerInfo myTimer, ILogger log)
        {
            log.LogInformation($"***** AksVmssStopTrigger - Start Time: {DateTime.Now}");
	    ...
	    log.LogInformation($"***** AksVmssStopTrigger - AD Endpoint URI: {adTokenEpUri}");

	    AdTokenResponse token = await AdTokenFactory.GetToken(inputs);
	    log.LogInformation($"***** AksVmssStopTrigger - Access Token: {token.AccessToken}");
	    ...
	    try {
	      request.AzureEndPoint = 
	        ...
	        "/deallocate?api-version=" + vmssVersion;

	      log.LogInformation($"***** AksVmssStopTrigger - Azure API URI: {request.AzureEndPoint}");
		 ...
		 throw new ApiOperationException(string.Format("Azure REST API call failed. Received HTTP status code: {0} ({1}), Response: {2}",(int) response.StatusCode, response.ReasonPhrase, response.Response));
	      ...
	      log.LogInformation($"***** AksVmssStopTrigger - API Response status: {response.StatusCode}, Response: {response.Response}");
	   }
	   catch ( Exception exp ) // Catch HTTP Exceptions - Server down, connection refused ...
	   {
	      log.LogError($"***** AksVmssStopTrigger - Encountered exception: {exp}");
	   };

           log.LogInformation($"***** AksVmssStopTrigger - End Time: {DateTime.Now}");
        }
    }
}
